...
export default function CustomerDashboard() {
  ...
  useEffect(() => {
    ...
    // Always fetch fresh data from server — never trust localStorage for live data
    fetchFreshData()
  }, [])
  ...
  const fetchFreshData = async () => {
    try {
      ...
      // Update localStorage with fresh data so name shows instantly on next login
      localStorage.setItem('customerUser', JSON.stringify(profileRes.data.customer))
    ...
    <main style={{ minHeight:'100vh', padding:'24px' }}>
      ...
      <div style={{ maxWidth:960, margin:'0 auto' }}>
        ...
        <div className="glass fade-up" style={{
          ...
          background:'linear-gradient(135deg,rgba(0,212,255,0.07),rgba(14,165,233,0.04))',
          ...
          <div style={{ flex:1 }}>
            ...
            {/* Payment status — always fresh from server */}
            {payment.totalCost > 0 && (
              <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                ...
                <span style={{
                  fontSize:'0.78rem', fontWeight:600, padding:'5px 12px', borderRadius:999,
                  background: hasDue ? 'rgba(239,68,68,0.08)' : 'rgba(16,185,129,0.08)',
          ...
          {orders.length === 0 ? (
            <div style={{ textAlign:'center', padding:'48px 0' }}>
              ...
              <p style={{ color:'#6B7280', fontSize:'0.9rem' }}>
                No orders yet. Visit the shop to place an order.
              </p>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:8, marginBottom:12 }}>
                      ...
                      <div style={{ textAlign:'right' }}>
                        ...
                        <p style={{ fontSize:'0.78rem', marginTop:5, fontWeight:600, color:days.color }}>
                          {days.label}
                        </p>
                      </div>
                    </div>
                      ...
                      <div style={{ marginTop:16, paddingTop:16, borderTop:'1.5px solid rgba(79,70,229,0.1)' }}>
                        ...
                        {order.alteration?.required && (
                          <div style={{ background:'rgba(245,158,11,0.06)', border:'1px solid rgba(245,158,11,0.2)', borderRadius:8, padding:'10px 14px' }}>
                            <p style={{ fontSize:'0.78rem', color:'#D97706', fontWeight:600, marginBottom:3 }}>
                              ⚠️ Alteration Required
                            </p>
                            <p style={{ fontSize:'0.82rem', color:'#4B5563' }}>
                              {order.alteration.notes || 'No notes'}
                            </p>
                          </div>
                        )}
