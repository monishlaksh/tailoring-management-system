          ...
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom:18 }}>
              ...
              <div style={{ position:'relative' }}>
                ...
                <input type="text" value={form.username} onChange={e => setForm({...form, username:e.target.value})} placeholder="Enter username" required style={inputStyle}
            ...
            <div style={{ marginBottom:26 }}>
              ...
              <div style={{ position:'relative' }}>
                ...
                <input type={showPass ? 'text' : 'password'} value={form.password} onChange={e => setForm({...form, password:e.target.value})} placeholder="Enter password" required style={{ ...inputStyle, paddingRight:44 }}
                  onFocus={e => { e.target.style.borderColor='#4F46E5'; e.target.style.boxShadow='0 0 0 4px rgba(79,70,229,0.08)' }}
                  onBlur={e => { e.target.style.borderColor='rgba(79,70,229,0.2)'; e.target.style.boxShadow='none' }} />
                <button type="button" onClick={() => setShowPass(!showPass)} style={{ position:'absolute', right:14, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', color:'#9CA3AF', cursor:'pointer', display:'flex' }}>
                  {showPass ? <EyeOff size={17} /> : <Eye size={17} />}
                </button>
              </div>
            </div>
            <button type="submit" disabled={loading} className="btn-primary" style={{ width:'100%', padding:'14px', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
              {loading ? <><div className="spinner" />Signing in...</> : 'Sign In to Dashboard'}
            </button>
          </form>
          <div style={{ marginTop:20, padding:'10px 14px', background:'rgba(79,70,229,0.05)', borderRadius:8, border:'1px solid rgba(79,70,229,0.1)' }}>
            <p style={{ fontSize:'0.76rem', color:'#6B7280', textAlign:'center' }}>
              🔐 Default: <strong style={{ color:'#4F46E5' }}>admin</strong> / <strong style={{ color:'#4F46E5' }}>admin123</strong>
            </p>
          </div>
        </div>
