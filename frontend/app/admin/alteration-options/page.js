...
export default function AlterationOptionsPage() {
  ...
  const [newForm, setNewForm]   = useState({ name:'', description:'', extraCost:0 })
  const [editForm, setEditForm] = useState({ name:'', description:'', extraCost:0 })
  ...
  const fetchOptions = async () => {
    try {
      const res = await API.get('/api/alteration-options/all')
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setNewForm({ name:'', description:'', extraCost:0 })
      setShowNew(false)
      fetchOptions()
    ...
    <main style={{ minHeight:'100vh', padding:'24px', maxWidth:900, margin:'0 auto' }}>
      ...
      <div className="glass" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 24px', marginBottom:24, flexWrap:'wrap', gap:12 }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            <h1 style={{ fontSize:'1rem', fontWeight:700, color:'#1E1B4B' }}>Alteration Options</h1>
            <p style={{ fontSize:'0.72rem', color:'#6B7280' }}>Manage alteration types shown during order creation</p>
          </div>
        </div>
      ...
      {showNew && (
        <div className="glass" style={{ padding:24, marginBottom:20, border:'1.5px solid rgba(79,70,229,0.2)' }}>
          <p style={{ fontWeight:700, color:'#1E1B4B', marginBottom:16, fontSize:'0.9rem' }}>New Alteration Option</p>
          <div style={{ display:'grid', gap:14 }}>
            <div>
              ...
              <input type="text" value={newForm.name} onChange={e => setNewForm({...newForm,name:e.target.value})}
                placeholder="e.g. Tight Fitting, Shorten Sleeve..." className="input-field" />
            </div>
            <div>
              <label className="input-label">DESCRIPTION</label>
              <textarea value={newForm.description} onChange={e => setNewForm({...newForm,description:e.target.value})}
                placeholder="Brief description of what this alteration involves..." rows={2}
          ...
          <div style={{ display:'flex', gap:10, marginTop:18 }}>
            <button onClick={handleCreate} disabled={savingNew} className="btn-primary"
              ...
              {savingNew ? <><div className="spinner" />Saving...</> : <><Check size={15} />Add Option</>}
            </button>
            <button onClick={() => { setShowNew(false); setNewForm({ name:'', description:'', extraCost:0 }) }}
              className="btn-ghost" style={{ padding:'11px 20px' }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="glass" style={{ textAlign:'center', padding:'48px' }}>
          ...
          <p style={{ color:'#6B7280', marginBottom:16 }}>No alteration options yet.</p>
          <button onClick={() => setShowNew(true)} className="btn-primary" style={{ padding:'10px 24px' }}>+ Add First Option</button>
        </div>
      ) : (
        <div style={{ display:'grid', gap:10 }}>
          {options.map(opt => (
            <div key={opt._id} className="glass"
              style={{ padding:'0', overflow:'hidden', border:opt.isActive?'1.5px solid rgba(255,255,255,0.8)':'1.5px solid rgba(239,68,68,0.15)' }}>
              ...
              {editing === opt._id ? (
                ...
                <div style={{ padding:'20px' }}>
                  ...
                  <div style={{ display:'flex', gap:8 }}>
                    <button onClick={() => handleUpdate(opt._id)} disabled={savingEdit}
                      style={{ padding:'9px 20px', background:'linear-gradient(135deg,#10B981,#059669)', color:'white', border:'none', borderRadius:10, fontFamily:'Poppins,sans-serif', fontWeight:600, fontSize:'0.85rem', cursor:'pointer', display:'flex', alignItems:'center', gap:6 }}>
                      {savingEdit ? <><div className="spinner" />Saving...</> : <><Check size={14} />Save</>}
                    </button>
                    <button onClick={() => setEditing(null)} className="btn-ghost" style={{ padding:'9px 16px' }}>Cancel</button>
                  </div>
                </div>
              ) : (
                /* View mode */
                <div style={{ padding:'16px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
                  <div style={{ flex:1 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:4 }}>
                      <div style={{ width:8, height:8, borderRadius:'50%', background:opt.isActive?'#10B981':'#EF4444', flexShrink:0 }} />
                      <span style={{ fontWeight:700, color:opt.isActive?'#1E1B4B':'#9CA3AF', fontSize:'0.95rem' }}>
                        {opt.name}
                      </span>
                      {(opt.extraCost||0) > 0 && (
                        <span style={{ fontSize:'0.75rem', fontWeight:700, color:'#059669', padding:'2px 8px', background:'rgba(16,185,129,0.1)', borderRadius:999 }}>
                          +₹{opt.extraCost.toLocaleString('en-IN')}
                        </span>
                      )}
                      {(opt.extraCost||0) === 0 && (
                        <span style={{ fontSize:'0.72rem', color:'#9CA3AF', padding:'2px 8px', background:'rgba(156,163,175,0.1)', borderRadius:999 }}>
                          No extra cost
                        </span>
                      )}
                    </div>
                    {opt.description && (
                      <p style={{ fontSize:'0.8rem', color:'#6B7280', marginLeft:18 }}>{opt.description}</p>
                    )}
                  </div>
                  <div style={{ display:'flex', gap:6 }}>
                    <button onClick={() => { setEditing(opt._id); setEditForm({ name:opt.name, description:opt.description||'', extraCost:opt.extraCost||0 }) }}
                    ...
                    <button onClick={() => handleToggle(opt)}
                      style={{ background:opt.isActive?'rgba(239,68,68,0.08)':'rgba(16,185,129,0.08)', border:`1px solid ${opt.isActive?'rgba(239,68,68,0.2)':'rgba(16,185,129,0.2)'}`, borderRadius:8, padding:'6px 12px', color:opt.isActive?'#DC2626':'#059669', fontSize:'0.78rem', fontWeight:600, cursor:'pointer', fontFamily:'Poppins,sans-serif' }}>
