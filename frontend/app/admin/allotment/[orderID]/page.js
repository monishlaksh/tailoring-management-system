...
function PaymentModal({ order, onClose, onSuccess }) {
  ...
  const cashTotal = Object.entries(breakdown).reduce((sum, [key, qty]) => {
    ...
    const note = NOTES_LIST.find(n => n.key === key)
  ...
  const handleSubmit = async () => {
    ...
    if (method === 'gpay' && !amount)         { setError('Enter amount'); return }
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(30,27,75,0.45)',
      ...
      <div style={{ background:'white', borderRadius:20, width:'100%', maxWidth:460,
        ...
        <div style={{ padding:'16px 20px', borderBottom:'1px solid #F3F4F6',
          ...
          <div>
            <p style={{ fontWeight:800, fontSize:'1rem', color:'#1E1B4B' }}>💳 Record Payment</p>
        ...
        <div style={{ padding:'16px 20px' }}>
          <div style={{ background:'#F8F7FF', borderRadius:12, padding:'12px', marginBottom:16 }}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
              {[
                { l:'Total',     v:`₹${totalCost.toLocaleString('en-IN')}`,   c:'#1E1B4B' },
                { l:'Paid',      v:`₹${alreadyPaid.toLocaleString('en-IN')}`,  c:'#059669' },
                { l:'Remaining', v:`₹${remaining.toLocaleString('en-IN')}`,
                  c: remaining > 0 ? '#DC2626' : '#059669' },
              ].map((s,i) => (
                <div key={i} style={{ textAlign:'center' }}>
                  <p style={{ fontSize:'0.62rem', color:'#9CA3AF', fontWeight:600, marginBottom:3 }}>{s.l}</p>
                  <p style={{ fontSize:'0.92rem', fontWeight:800, color:s.c }}>{s.v}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {remaining === 0 ? (
            <div style={{ textAlign:'center', padding:'24px 0' }}>
              ...
              <p style={{ fontWeight:700, color:'#059669' }}>Fully Paid!</p>
            </div>
          ) : (
            <>
                ...
                {[
                  ...
                  <button key={m.v} type="button" onClick={() => setMethod(m.v)}
                    style={{ padding:'12px', borderRadius:12, cursor:'pointer',
                      border: method===m.v ? `2px solid ${m.b}` : '1.5px solid #E5E7EB',
                      background: method===m.v ? m.bg : 'white',
                      fontFamily:'Poppins,sans-serif', fontWeight:700, fontSize:'0.9rem',
                      color: method===m.v ? m.c : '#6B7280' }}>
                    {m.l}
                  </button>
                ))}
              </div>
              ...
              {method === 'cash' && (
                <div>
                  <div style={{ display:'grid', gap:7, marginBottom:12 }}>
                    {NOTES_LIST.map(note => (
                      <div key={note.key} style={{ display:'flex', alignItems:'center',
                        gap:10, background:'#F9FAFB', borderRadius:10, padding:'9px 12px' }}>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:7, flex:1 }}>
                          <button type="button"
                            onClick={() => setNote(note.key, (breakdown[note.key]||0)-1)}
                            style={{ width:28, height:28, borderRadius:'50%',
                              background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.2)',
                              cursor:'pointer', color:'#DC2626', fontSize:'1.1rem', fontWeight:800,
                              display:'flex', alignItems:'center', justifyContent:'center' }}>−</button>
                          <input type="number" min="0" value={breakdown[note.key]||''}
                            onChange={e => setNote(note.key, e.target.value)} placeholder="0"
                            style={{ width:42, textAlign:'center', padding:'5px 2px',
                          ...
                          <button type="button"
                            ...
                            style={{ width:28, height:28, borderRadius:'50%',
                              ...
                              cursor:'pointer', color:'#059669', fontSize:'1.1rem', fontWeight:800,
                              display:'flex', alignItems:'center', justifyContent:'center' }}>+</button>
                        </div>
                        <p style={{ fontSize:'0.85rem', fontWeight:700, color:'#1E1B4B', minWidth:46, textAlign:'right' }}>
                          {(breakdown[note.key]||0)>0 ? `₹${(note.value*breakdown[note.key]).toLocaleString('en-IN')}` : '—'}
                        </p>
                      </div>
                    ))}
                    <div style={{ display:'flex', alignItems:'center', gap:10,
                      background:'#F9FAFB', borderRadius:10, padding:'9px 12px' }}>
                      <div style={{ width:54, textAlign:'center', background:'rgba(245,158,11,0.08)',
                        border:'1px solid rgba(245,158,11,0.2)', borderRadius:8, padding:'4px 6px' }}>
                        <p style={{ fontSize:'1rem' }}>🪙</p>
                      </div>
                      <div style={{ flex:1, display:'flex', alignItems:'center', gap:5 }}>
                        <span style={{ color:'#6B7280' }}>₹</span>
                        <input type="number" min="0" step="0.5" value={breakdown.coins||''}
                          onChange={e => setBreakdown(p=>({...p,coins:parseFloat(e.target.value)||0}))}
                          placeholder="0"
                          style={{ flex:1, padding:'7px 8px', border:'1.5px solid #E5E7EB',
                            borderRadius:8, fontFamily:'Poppins,sans-serif',
                            fontSize:'0.95rem', fontWeight:700, color:'#1E1B4B', outline:'none' }}/>
                      </div>
                      <p style={{ fontSize:'0.85rem', fontWeight:700, color:'#1E1B4B', minWidth:46, textAlign:'right' }}>
                        {(breakdown.coins||0)>0 ? `₹${breakdown.coins}` : '—'}
                      </p>
                    </div>
                  </div>
                  <div style={{ background: cashTotal>=remaining ? 'rgba(16,185,129,0.06)' : 'rgba(245,158,11,0.06)',
                    border:`1.5px solid ${cashTotal>=remaining ? 'rgba(16,185,129,0.2)' : 'rgba(245,158,11,0.2)'}`,
                    borderRadius:12, padding:'12px 14px', marginBottom:14 }}>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
                      {[
                        { l:'Cash Given', v:`₹${cashTotal.toLocaleString('en-IN')}`, c:'#1E1B4B' },
                        { l:'Amount Due', v:`₹${remaining.toLocaleString('en-IN')}`, c:'#DC2626' },
                        { l:change>0?'↩ Return':'Change', v:change>0?`₹${change.toLocaleString('en-IN')}`:'—', c:change>0?'#D97706':'#9CA3AF' },
                      ].map((s,i) => (
                        <div key={i} style={{ textAlign:'center' }}>
                          <p style={{ fontSize:'0.62rem', color:'#9CA3AF', fontWeight:600, marginBottom:2 }}>{s.l}</p>
                          <p style={{ fontSize:'0.9rem', fontWeight:800, color:s.c }}>{s.v}</p>
                        </div>
                      ))}
                    </div>
                    {change>0 && <p style={{ textAlign:'center', fontSize:'0.75rem', color:'#D97706', fontWeight:600, marginTop:8 }}>⚠️ Return ₹{change.toLocaleString('en-IN')} to customer</p>}
                  </div>
                </div>
              )}
              ...
              {method === 'gpay' && (
                <div style={{ marginBottom:14 }}>
                  <div style={{ textAlign:'center', padding:'16px', background:'rgba(79,70,229,0.05)', borderRadius:12, marginBottom:14 }}>
                    <p style={{ fontSize:'2rem', marginBottom:6 }}>📱</p>
                    <p style={{ fontSize:'0.85rem', color:'#4F46E5', fontWeight:600 }}>GPay / UPI Payment</p>
                  </div>
                  <label style={{ fontSize:'0.72rem', fontWeight:700, color:'#9CA3AF', textTransform:'uppercase', display:'block', marginBottom:6 }}>Amount Received (₹)</label>
                  <input type="number" min="0" value={amount} onChange={e => setAmount(parseFloat(e.target.value)||0)}
                    style={{ width:'100%', padding:'12px 14px', border:'1.5px solid rgba(79,70,229,0.2)', borderRadius:10, fontFamily:'Poppins,sans-serif', fontSize:'1.1rem', fontWeight:700, color:'#4F46E5', outline:'none', marginBottom:12 }}/>
                  <label style={{ fontSize:'0.72rem', fontWeight:700, color:'#9CA3AF', textTransform:'uppercase', display:'block', marginBottom:6 }}>Reference / UTR (optional)</label>
                  <input type="text" value={gpayRef} onChange={e => setGpayRef(e.target.value)} placeholder="e.g. UPI123456789"
                    style={{ width:'100%', padding:'11px 14px', border:'1.5px solid rgba(79,70,229,0.2)', borderRadius:10, fontFamily:'Poppins,sans-serif', fontSize:'0.9rem', color:'#1E1B4B', outline:'none' }}/>
                </div>
              )}

              <div style={{ marginBottom:14 }}>
                <label style={{ fontSize:'0.72rem', fontWeight:700, color:'#9CA3AF', textTransform:'uppercase', display:'block', marginBottom:6 }}>Notes (optional)</label>
                <input type="text" value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g. Partial payment..."
                  style={{ width:'100%', padding:'10px 14px', border:'1.5px solid #E5E7EB', borderRadius:10, fontFamily:'Poppins,sans-serif', fontSize:'0.88rem', color:'#1E1B4B', outline:'none' }}/>
              </div>

              {error && <p style={{ color:'#DC2626', fontSize:'0.82rem', marginBottom:12 }}>⚠️ {error}</p>}

              <button type="button" onClick={handleSubmit}
                disabled={saving||(method==='cash'&&cashTotal===0)||(method==='gpay'&&!amount)}
                style={{ width:'100%', padding:'14px',
                  background:(method==='cash'&&cashTotal===0)||(method==='gpay'&&!amount) ? '#E5E7EB'
                    : method==='cash' ? 'linear-gradient(135deg,#059669,#10B981)'
                    : 'linear-gradient(135deg,#4F46E5,#6366F1)',
                  color:(method==='cash'&&cashTotal===0)||(method==='gpay'&&!amount) ? '#9CA3AF' : 'white',
                  border:'none', borderRadius:12, fontFamily:'Poppins,sans-serif',
                  fontWeight:700, fontSize:'0.95rem', cursor:'pointer',
                  display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
                {saving ? '⏳ Recording...'
                  : method==='cash' ? `✅ Confirm ₹${Math.min(cashTotal,remaining).toLocaleString('en-IN')} Cash`
