    ...
    <main style={{ minHeight:'100vh', padding:'20px', maxWidth:1100, margin:'0 auto' }}>
      ...
      <div className="glass" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 22px', marginBottom:20, flexWrap:'wrap', gap:12, borderTop:'3px solid #F59E0B' }}>
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
          <button onClick={() => router.push('/employee/customers/new')}
            style={{ display:'flex', alignItems:'center', gap:5, background:'rgba(245,158,11,0.1)', border:'1.5px solid rgba(245,158,11,0.3)', borderRadius:8, padding:'8px 14px', color:'#D97706', fontSize:'0.8rem', fontWeight:600, cursor:'pointer', fontFamily:'Poppins,sans-serif' }}>
            <Plus size={14} /> Add Customer
          </button>
          <button onClick={() => router.push('/employee/orders/new')}
            style={{ display:'flex', alignItems:'center', gap:5, background:'linear-gradient(135deg,#F59E0B,#D97706)', border:'none', borderRadius:8, padding:'8px 16px', color:'white', fontSize:'0.8rem', fontWeight:600, cursor:'pointer', fontFamily:'Poppins,sans-serif', boxShadow:'0 4px 12px rgba(245,158,11,0.3)' }}>
      ...
      <div style={{ background:'rgba(245,158,11,0.06)', border:'1.5px solid rgba(245,158,11,0.2)', borderRadius:12, padding:'12px 18px', marginBottom:20, display:'flex', alignItems:'center', gap:10 }}>
        ...
        <p style={{ fontSize:'0.82rem', color:'#92400E' }}>
          You can see and manage only the orders <strong>you have created</strong>. Order details cannot be edited after creation — only status can be updated.
        </p>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14, marginBottom:20 }}>
        {[
          ...
          <div key={i} className="glass" style={{ padding:'18px 16px', background:c.bg, border:`1.5px solid ${c.border}` }}>
      ...
      <div className="glass" style={{ padding:'22px' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:18, flexWrap:'wrap', gap:12 }}>
          <h2 style={{ fontSize:'0.95rem', fontWeight:700, color:'#1E1B4B' }}>My Orders</h2>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign:'center', padding:'48px 0' }}>
            ...
            <p style={{ color:'#6B7280', fontSize:'0.9rem', marginBottom:16 }}>No orders yet. Create your first order!</p>
            <button onClick={() => router.push('/employee/orders/new')}
              style={{ padding:'10px 24px', background:'linear-gradient(135deg,#F59E0B,#D97706)', color:'white', border:'none', borderRadius:10, fontFamily:'Poppins,sans-serif', fontWeight:600, fontSize:'0.85rem', cursor:'pointer' }}>
              + Create Order
            </button>
          </div>
        ) : (
                ...
                {filtered.map(order => (
                  <tr key={order._id}>
                    {[
                      ...
                      <td key={ci} style={{ padding:'11px 12px', background:'rgba(255,255,255,0.6)', borderRadius: ci===0?'10px 0 0 10px':ci===6?'0 10px 10px 0':0 }}>{cell}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
