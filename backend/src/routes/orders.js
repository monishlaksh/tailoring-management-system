...
const { protect, protectEmployee, protectAdminOrEmployee, protectCustomer } = require('../middleware/auth')
...
const getNextOrderID = async () => {
  ...
  let counter = next
  while (exists) { counter++; newID = `ORD${String(counter).padStart(6,'0')}`; exists = await Order.findOne({ orderID: newID }).lean() }
  return newID
}
...
router.get('/stats/dashboard', protect, async (req, res) => {
  try {
    ...
    const [total,booking,cutting,stitching,finishing,ready,todayDelivery,delayed] = await Promise.all([
...
// Get all orders — admin sees all, employee sees only their own
...
router.post('/', protectAdminOrEmployee, async (req, res) => {
  try {
    const { customerID, clothType, quantity, fabricNotes, specialInstructions, measurements, alteration, deliveryDate, referenceImage } = req.body
    if (!customerID) return res.status(400).json({ success:false, message:'Customer ID required' })
    if (!clothType)  return res.status(400).json({ success:false, message:'Cloth type required' })
    if (!deliveryDate) return res.status(400).json({ success:false, message:'Delivery date required' })
    ...
    const order = await Order.create({
      orderID, customerID, customerRef: customer._id,
      clothType, quantity: quantity||1,
      fabricNotes: fabricNotes||'', specialInstructions: specialInstructions||'',
      measurements: measurements||{}, alteration: alteration||{ required:false, notes:'' },
      deliveryDate, referenceImage: referenceImage||'',
      createdBy,
    })
